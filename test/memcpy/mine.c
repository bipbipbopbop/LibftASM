...
int	main(void)
{
	...
	puts("call to ft_memcpy(str, \"aaaaa\", 5): ");
	for (int i = 0; i < 10; ++i)
		printf("%#.2x ", str[i]);
	putchar('\n');

	ft_memcpy(str + 3, "EEEE", 4);
	puts("call to ft_memcpy(str + 3, \"EEEE\", 4):");
	for (int i = 0; i < 10; ++i)
		printf("%#.2x ", str[i]);
	putchar('\n');

	ft_memcpy(str, "ZZ", 0);
	puts("call to ft_memcpy(str, \"ZZ\", 0):");
	for (int i = 0; i < 10; ++i)
		printf("%#.2x ", str[i]);
	putchar('\n');

	signal(SIGSEGV, catch_segv);// ft_memcpy(NULL, NULL, ...) doesn't segv
	puts("testing ft_memcpy(NULL, NULL, 2)");
	ft_memcpy(NULL, NULL, 2);
	puts("testing ft_memcpy(str, NULL, 2)");
	ft_memcpy(str, NULL, 2);
	puts("testing ft_memcpy(NULL, \"kek\", 2)");
